...
def process_file(fn, chunk_len=20480, drop_last=True):
    ...
    data = [np.expand_dims(data[k:k+chunk_len], 0) for k in range(0,
                                                                  len(data) - (chunk_len + 1 if drop_last else 0), chunk_len)]
...
class LibriSpeechDataset(torch.utils.data.Dataset):
    def __init__(self, file_list, chunk_len=20480):
        ...
        self.indices = [(i, j) for i in range(len(self.all_files))
                        for j in range(self.lens[i])]

        self.data_by_index = [tuple(
            map(int, s.split("/")[-1].split(".")[0].split("-"))) for s in self.all_files]
    ...
    def __getitem__(self, idx):
        ...
        record = process_file(
            self.all_files[file], chunk_len=self.chunk_len)[0][chunk]
